<script lang="ts">
    ...
    function createTask() {
        ...
        const t = taskStore.addTask({ name: newName.trim(), description: newDesc.trim(), goal: newGoal.trim() });
        showCreate = false;
        selected = t;
    }
...
$: if (!showCreate) {
    ...
    function sessionsFor(taskId: string) {
        ...
        return allSessions.filter((s: any) => s.taskId === taskId);
    }
</script>
        ...
        <div class="layout">
            ...
            <div class="list">
                ...
                {:else}
                    <div class="grid">
                        {#each tasks as task}
                            <div class="grid-item" role="button" tabindex="0" on:click={() => selectTask(task)} on:keydown={(e) => (e.key === 'Enter' || e.key === ' ') && selectTask(task)} aria-label={`Open ${task.name}`}>
                                <TaskCard {task} loading={starting[task.id]} on:start={(ev) => startSessionFromCard(ev.detail.task)} on:edit={(ev) => selectTask(ev.detail.task)} />
                            </div>
                        {/each}
                    </div>
                {/if}
                    ...
                    <Popup bind:isOpen={showCreate} title="Create new task" onClose={() => (showCreate = false)}>
                        <form id="task-create-dialog" on:submit|preventDefault={createTask} class="create-form" aria-labelledby="create-task-label">
                            ...
                            <div class="field">
                                ...
                                <textarea id="task-desc" rows="4" bind:value={newDesc} maxlength="600" placeholder="More detail about this task (optional)"></textarea>
                            </div>
            ...
            <aside class="detail">
                {#if selected}
                    <div class="card detail-card">
                        <h2>{selected.name}</h2>
                        <p class="goal"><strong>Goal:</strong> {selected.goal || 'No goal provided'}</p>
                        <p class="desc">{selected.description || 'No description'}</p>

                        <h3>Sessions</h3>
                        {#if sessionsFor(selected.id).length === 0}
                            <p>No sessions started for this task yet.</p>
                        {:else}
                            <ul class="sessions-list">
                                {#each sessionsFor(selected.id) as s}
                                    <li class="session-item card">Session: {s.id} • {new Date(s.startTime).toLocaleString()}</li>
                                {/each}
                            </ul>
                        {/if}
                    </div>
                {:else}
                    <div class="card detail-card muted">Select a task to see details & sessions.</div>
                {/if}
            </aside>
        </div>
    {/if}
...
<style>
    ...
    .create-form input, .create-form textarea { padding:.5rem; border-radius:6px; border:1px solid var(--color-border); background:var(--color-card-bg); color:var(--color-text-primary); }
    ...
    .create-floating .btn.create { padding: .6rem 1rem; box-shadow: 0 6px 20px rgba(12,12,12,0.2); border-radius: 9999px; border: none; background: var(--color-accent); color: white; font-weight: 700; }
    ...
    @media (max-width: 768px) {
        ...
        .create-floating .btn.create { padding: .7rem 1.1rem; }
    ...
    .muted { color:var(--color-text-secondary); padding:1.25rem; text-align:center; }
    .sessions-list { margin:0; padding:0; list-style:none; display:flex; flex-direction:column; gap:.5rem; }
    .session-item { padding:.5rem; }

    @media (min-width: 1200px) {
        /* On very wide screens, allow the detail panel to sit beside the list again if desired */
        .layout { flex-direction:column; /* keep stacked by default */ }
        .detail { width:100%; }
    }
</style>
