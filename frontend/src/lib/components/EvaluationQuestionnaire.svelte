<script lang="ts" context="module">
    export interface Question {
        id: string;
        label: string;
        type: 'text' | 'textarea' | 'select' | 'number' | 'none';
        options?: string[];
        stars?: boolean;
...
<script lang="ts">
    ...
    const validateField = (questionId: string): boolean => {
        ...
        if (question.required) {
            ...
            else if (question.stars && (!response?.rating || response.rating === 0)) {
                error = 'Please provide a rating';
            }
        ...
        if (!error && response?.answer && question.type !== 'none') {
...
<form onsubmit={submitForm} class="space-y-6" novalidate>
        ...
        {#each template as q}
            <div class="my-4">
                <div class="card" class:has-error={touched[q.id] && errors[q.id]}>
                    ...
                    <!-- Only show input fields if type is not 'none' -->
                    {#if q.type !== 'none'}
                        {#if q.type === 'text' || q.type === 'number'}
                            <input
                                ...
                                onblur={() => { touched[q.id] = true; validateField(q.id); }}
                        ...
                        {:else if q.type === 'textarea'}
                            <textarea
                                ...
                                class="input-field w-full"
                                class:error={touched[q.id] && errors[q.id]}
                                oninput={(e) => updateAnswer(q.id, (e.target as HTMLTextAreaElement).value)}
                                onblur={() => { touched[q.id] = true; validateField(q.id); }}
                                value={responses[q.id]?.answer ?? ''}
                                rows="3"
                                aria-invalid={touched[q.id] && errors[q.id] ? 'true' : 'false'}
                    ...
                    <!-- Show rating section if stars is enabled OR if type is 'none' -->
                    {#if q.stars || q.type === 'none'}
                        <div class="mt-3" class:rating-only={q.type === 'none'}>
                            {#if q.type === 'none'}
                                <div class="rating-only-label">
                            ...
                            {:else if q.starType === 'slider'}
                                <div class="slider-container">
                                    <input
                                        ...
                                        oninput={(e) => setRating(q.id, parseInt((e.target as HTMLInputElement).value))}
                                    ...
                                    <div class="text-center mt-2 font-semibold">
                                        ...
                                        {#if responses[q.id]?.rating && (responses[q.id]?.rating ?? 0) > 0}
